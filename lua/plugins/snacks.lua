return {
	...
	opts = {
		...
		scroll = {
			enabled = true,
			animate = { duration = { step = 10, total = 200 }, easing = "inOutSine" },
			animate_repeat = {
				delay = 100,
	...
	keys = {
		...
		{
			"<C-\\>",
			"<Esc><Cmd>lua Snacks.terminal.toggle()<CR>",
			mode = "i",
			desc = "Terminal toggle",
		},
		{
		...
		{
			"<A-\\>",
			"<Esc><Cmd>lua Snacks.terminal.toggle(nil, { win = { position = 'right' } })<CR>",
			mode = "i",
			desc = "Terminal toggle (vertical)",
		},
		{
		...
		{
			"<A-d>",
			"<Esc><Cmd>lua Snacks.terminal.toggle(nil, { win = { position = 'float' } })<CR>",
			mode = "i",
			desc = "Terminal toggle (float)",
		},
		{
			"<leader>gg",
			function()
				Snacks.terminal.toggle("lazygit", { win = { position = "float" } })
			end,
			desc = "Lazygit",
		},
		{
		...
		-- legacy: command panel / visual grep
		{
			"<C-p>",
			function()
				Snacks.picker.keymaps()
